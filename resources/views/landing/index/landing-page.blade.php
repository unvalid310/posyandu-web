    ...
    <div id="carouselExampleCaptions" class="carousel slide carousel-fade" data-bs-ride="carousel">
        ...
        <div class="carousel-inner ratio ratio-16x9">
            @foreach ($informasi_populer as $item)
                <div class="carousel-item @if ($loop->iteration == 1) active @endif" data-bs-interval="1000">
                    <img src="{{ route('informasi_penting.get_img', $item->id) }}" class="d-block w-100" alt="...">
                    <div class="carousel-caption d-none d-md-block">
                        <h5 class="text-primary fw-bold h2"><a class="text-decoration-none"
                                href="{{ route('Detail Berita', $item->slug) }}">{{ $item->judul_informasi }}</a></h5>
                    </div>
                </div>
            @endforeach
        </div>
            ...
            <div class="row">
                <div class="col-xl-8 mx-auto">
                    <div class="section-title text-center mb-55">
                        ...
                        <p class="wow fadeInUp" data-wow-delay=".4s">Simak berita terkini terkait kesehatan dan jangan
                            lewatkan informasi mengenai kegiatan-kegiatan yang dilaksanakan oleh Posyandu</p>
                    </div>
                </div>
            </div>
            <div class="row">
                        ...
                        <div class="single-blog mb-30 wow fadeInUp" data-wow-delay=".2s">
                            <div class="blog-img">
                                <a href="{{ route('Detail Berita', $item->slug) }}"><img
                                        src="{{ route('informasi_penting.get_img', $item->id) }}" alt=""></a>
                            </div>
                            <div class="blog-content">
                                <h4><a class="text-decoration-none"
                                        href="{{ route('Detail Berita', $item->slug) }}">{{ $item->judul_informasi }}</a>
                                </h4>
                                <p>{{ strip_tags(substr($item->informasi, 0, 100)) }} ...</p>
                                <a class="read-more text-decoration-none"
                                    href="{{ route('Detail Berita', $item->slug) }}">Baca selengkapnya <i
                ...
                @endforeach
                <div class="section-title text-center mb-55">
                    <span class="wow fadeInDown text-center" data-wow-delay=".2s"><a class="text-decoration-none"
                            href="{{ route('Berita') }}">Lihat Semua Berita</a></span>
                </div>
            </div>
        </div>
    </section>
