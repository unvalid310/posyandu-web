...
@section('content')
    ...
    <div>
        ...
        <form class="d-flex flex-row justify-content-between">
                ...
                <div>
                    <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <a class="nav-link <?= empty($_GET['l']) || $_GET['l'] === 'pemeriksaan' ? 'active' : '' ?>"
                                aria-current="page" href="?l=pemeriksaan">Pemeriksaan</a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link <?= isset($_GET['l']) && $_GET['l'] === 'vitamin' ? 'active' : '' ?>"
                                aria-current="page" href="?l=vitamin">Vitamin</a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link <?= isset($_GET['l']) && $_GET['l'] === 'konsul' ? 'active' : '' ?>"
                                aria-current="page" href="?l=konsul">Konsultasi</a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link <?= isset($_GET['l']) && $_GET['l'] === 'imunisasi' ? 'active' : '' ?>"
                                aria-current="page" href="?l=imunisasi">Imunisasi</a>
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <div>
                <button type="button" class="btn btn-outline-primary" disabled id="filter_button" data-bs-toggle="modal"
                    data-bs-target="#modalFilter" data-bs-whatever="@mdo">
        ...
        <div>
            ...
            @if ($user_role === 'super admin' || $user_role === 'tenaga kesehatan')
                <form class="d-flex flex-row justify-content-between">
                    ...
                    <div>
                        <button type="button" class="btn btn-outline-primary" id="filter_button_tabel" disabled
                            data-bs-toggle="modal" data-bs-target="#modalFilterTable">
        ...
        <div class="modal fade" id="modalFilterTable" tabindex="-1" aria-labelledby="ModalFilterTabel"
            aria-hidden="true">
            <div class="modal-dialog col-3">
                <div class="modal-content">
                    <div class="modal-header">
...
@push('js')
    <script>
            ...
            success: (data) => {
                data.map((val, i) => {
                    $('#posyandu_laporan_filter_super_admin')
                        .append(
                            `<option value="${val.id}" ${i === 0 && "selected"}>${val.posyandu}</option>`
                            )
                })
            }
        ...
        $.ajax({
            ...
            success: (res) => {
                res['year'].map(val => {
                    ...
                    $('#_year_selection_')
                        .append(
                            `<option value="${val}" ${isSelected === true ? "selected" : "" } >${val}</option>`
                            )
                })
                res['month'].map((val, i) => {
                    ...
                    $('#_month_selection_last_')
                        .append(
                            `<option value="${i+1}" ${isSelected === true ? "selected" : "" } >${val}</option>`
                            )
                })
            }
        }).done(() => {
        ...
        $('#filter_type_tabel').on('change', () => {
            ...
            $.ajax({
                url: '/admin/ajax/tabel/filter?tk={{ $user_role === 'tenaga kesehatan' ? 1 : 0 }}&type=' +
                    valueType,
                method: 'GET',
                success: (res) => {
