<footer class="footer img-bg fixed-bottom"
    style="background-image:url('/index-template/img/bg/footer-bg.jpg'); height: 7vh">
    {{-- <div class="container"> --}}
    {{-- <div class="footer-widget-wrapper"> --}}
    {{-- <div class="row">
                <div class="col-xl-6 col-lg-6 col-md-12">
                    <div class="footer-widget mb-30">
                        <a href="index.html" class="logo">
                            <img src="{{ asset('/images/sipandu-logo.ico') }}" alt="" width="90" height="90" class="d-inline-block align-top" alt="Logo Posyandu">
                        </a>
                        <p>Lorem ipsum dolor serit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut
                            labore dolore magna
                            aliquyam erat diam voluptua.</p>
                        <div class="footer-social-links">
                            <ul>
                                <li><a href="#"><i class="lni lni-facebook-filled"></i></a></li>
                                <li><a href="#"><i class="lni lni-twitter-filled"></i></a></li>
                                <li><a href="#"><i class="lni lni-instagram-original"></i></a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="col-xl-3 col-lg-3 col-md-6">
                    <div class="footer-widget mb-30">
                        <h4>Menu</h4>
                        <ul class="footer-links">
                            <li><a class="text-decoration-none" href="#">Beranda</a></li>
                            <li><a class="text-decoration-none" href="#">Berita</a></li>
                            <li><a class="text-decoration-none" href="#">Tentang</a></li>
                            <li><a class="text-decoration-none" href="#">Kontak</a></li>
                            <li><a class="text-decoration-none" href="#">Regiatrasi</a></li>
                            <li><a class="text-decoration-none" href="#">Masuk</a></li>
                        </ul>
                    </div>
                </div>
                <div class="col-xl-3 col-lg-3 col-md-5">
                    <div class="footer-widget mb-30">
                        <h4>Pintasan</h4>
                        <ul class="footer-links">
                            <li><a class="text-decoration-none" href="#">Cardiology</a></li>
                            <li><a class="text-decoration-none" href="#">Neurology</a></li>
                            <li><a class="text-decoration-none" href="#">Gastroenterology</a></li>
                            <li><a class="text-decoration-none" href="#">Routine Checkup</a></li>
                            <li><a class="text-decoration-none" href="#">Orthopedics</a></li>
                            <li><a class="text-decoration-none" href="#">Dental Surgery</a></li>
                        </ul>
                    </div>
                </div>
            </div> --}}
    {{-- </div> --}}
    {{-- </div> --}}
    <div class="copyright-area border-0 p-0">
        <p class="mb-0 text-center my-2">Copyright &copy; {{ date('Y', strtotime(date('Y-m-d H:i:s'))) }}
            <a class="text-decoration-none" href="" rel="nofollow">Posyandu</a>
        </p>
    </div>
</footer>
